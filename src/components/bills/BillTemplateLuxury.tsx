  ...
  return (
    <div id="printable-bill-area" className="w-full max-w-[800px] mx-auto bg-white text-slate-900 p-8 shadow-2xl font-sans rounded-xl border border-gold-500/30">
      ...
      <div className="bg-[#fcfaf2] border border-[#f5e7b8] rounded-lg p-4 mb-6 flex justify-between">
        ...
        <div className="text-right">
          <p className="text-xs font-bold text-[#785816] uppercase tracking-wider">Payment Status</p>
      ...
      <div className="flex justify-between items-start pt-4 border-t border-slate-200 mb-6">
        <div className="w-1/2 pr-6">
          ...
          {settings.showTermsOnBill && (
            <div className="mt-4">
              ...
              <p className="text-[9px] text-slate-500 whitespace-pre-line mt-1">{settings.termsAndConditions}</p>
            </div>
          )}
        </div>
        ...
        <div className="w-1/2 pl-6 space-y-1.5 text-xs text-slate-700">
          ...
          {bill.calculation.itemDiscounts > 0 && (
            <div className="flex justify-between py-1 border-b border-slate-100 text-emerald-600">
              <span>Item Discounts</span>
              <span>-{formatCurrency(bill.calculation.itemDiscounts)}</span>
            </div>
          )}
          {settings.showGstOnBill && (
            <>
              <div className="flex justify-between py-1 border-b border-slate-100">
                <span>CGST (9%)</span>
                <span>{formatCurrency(bill.calculation.cgst)}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100">
                <span>SGST (9%)</span>
                <span>{formatCurrency(bill.calculation.sgst)}</span>
              </div>
            </>
          )}
          ...
          {bill.calculation.dueAmount > 0 && (
            <div className="flex justify-between py-1 font-bold text-rose-600 bg-rose-50 px-2 rounded">
      ...
      <div className="text-center pt-6 border-t border-slate-200">
        <p className="text-xs font-serif font-bold text-[#926f1a]">Thank you for your patronage</p>
        <p className="text-[10px] text-slate-400 mt-1">Computer Generated Tax Invoice. No Signature Required.</p>
      </div>
    </div>
  );
};
